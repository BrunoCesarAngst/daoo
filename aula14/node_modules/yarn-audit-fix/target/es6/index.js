 ...
 */
const printRuntimeDigest = ({ temp, cwd, flags, manifest }) => {
    if (flags.silent) {
        return;
    }
 ...
 * @return {void}
 */
const createTempAssets = ({ temp, flags }) => {
 ...
 * @return {void}
 */
const createSymlinks = ({ temp, flags, cwd, manifest }) => {
    const symlinkType = getSymlinkType(flags.symlink); // TODO fix fs-extra typings issue
    const workspaces = getWorkspaces(cwd, manifest);
    const links = [join(cwd, 'node_modules'), ...workspaces];
    links.forEach((pkgPath) => {
 ...
 * @return {void}
 */
const npmAuditFix = ({ temp, flags, cwd, manifest }) => {
    const requireNpmBeta = !!manifest.workspaces;
    const npm = getNpm(requireNpmBeta, flags['npm-v7'], flags.silent);
    const defaultFlags = {
        'package-lock-only': true,
    };
    const auditFlags = formatFlags(Object.assign(Object.assign({}, defaultFlags), flags), 'package-lock-only', 'verbose', 'loglevel', 'only', 'force', 'audit-level', 'silent', 'registry');
    const auditArgs = [
        'audit',
        'fix',
        ...auditFlags,
 ...
 * @return {void}
 */
const clear = ({ temp }) => fs.emptyDirSync(temp);
export const stages = [
    [
        'Runtime digest',
        printRuntimeDigest,
 ...
 */
export const run = (flags = {}) => __awaiter(void 0, void 0, void 0, function* () {
